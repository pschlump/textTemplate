...
func TestEvalFieldErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			"{{.MissingField}}",
			(*T)(nil),
			// "can't evaluate field MissingField in type *testTemplate.T", // PJS
			"template: tmpl:1:2: executing \"tmpl\" at <.MissingField>: can't evaluate field MissingField in type *textTemplate.T", // PJS
		},
		{
		...
		},
		{
			...
			"{{.X}}",
			(*T)(nil),
			// "nil pointer evaluating *template.T.X", // PJS
			"template: tmpl:1:2: executing \"tmpl\" at <.X>: nil pointer evaluating *textTemplate.T.X", // PJS
		},
		{
			"MissingKeyOnNilMap",
			"{{.MissingKey}}",
...
func TestExecutePanicDuringCall(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// `template: t:1:23: executing "t" at <.NonEmptyInterfaceNil.Method0>: nil pointer evaluating template.I.Method0`, // PJS
			`template: t:1:23: executing "t" at <.NonEmptyInterfaceNil.Method0>: nil pointer evaluating textTemplate.I.Method0`, // PJS
		},
	}
	for _, tc := range tests {
		b := new(bytes.Buffer)
